...
- [x] **Duplicate `stream.write_all`** (`main.rs:143-153`) — Fixed: introduced `keep_alive` variable to avoid repeating `!request.close_connection`; `write_all` appears once unconditionally, header insertion and `break` are each gated on `!keep_alive`.
